...
//  digitalWrite(redLed, LED_OFF);  // Make sure led is off
  ...
  Serial.println(F("Bienvenidos"));   // For debugging purposes
...
//      digitalWrite(redLed, LED_OFF);
  ...
  Serial.println(F("Pase la tarjeta..."));
...
//      digitalWrite(redLed, LED_ON);  // Make sure led is off
  ...
  if (programMode) {
    if ( isMaster(readCard) ) { //When in program mode check First If master card scanned again to exit program mode
      Serial.println(F("Tarjeta aceptada"));
      Serial.println(F("Saliendo del modo configuracion"));
      Serial.println(F("-----------------------------"));
      programMode = false;
      return;
    }
      ...
      if ( findID(readCard) ) { // If scanned card is known delete it
        Serial.println(F("Removiendo acceso..."));
        deleteID(readCard);
        Serial.println("-----------------------------");
        Serial.println(F("Pase una tarjeta para agregar o remover acceso"));
      }
      else {                    // If scanned card is not known add it
        Serial.println(F("Agregando acceso..."));
        writeID(readCard);
        Serial.println(F("-----------------------------"));
        Serial.println(F("Pase una tarjeta para agregar o remover acceso"));
      }
    ...
    if ( isMaster(readCard)) {    // If scanned card's ID matches Master Card's ID - enter program mode
      ...
      Serial.println(F("Acceso Aprobado!"));
      Serial.println(F("Hola Israel, Anderson o Juan - Modo Configuracion"));
      uint8_t count = EEPROM.read(0);   // Read the first Byte of EEPROM that
      Serial.print(F("Hay "));     // stores the number of ID's in EEPROM
      Serial.print(count);
      Serial.print(F(" record(s) en la EEPROM"));
      Serial.println("");
      Serial.println(F("Pasa una tarjeta para agregar o remover acceso."));
      Serial.println(F("Pasa la tarjeta principal nuevamente para salir del modo de configuracion"));
      Serial.println(F("-----------------------------"));
    }
    else {
...
//  Serial.print(v, HEX);
  if (v == 0x91)
    ...
    Serial.print(F("Manejo de acceso:"));
  Serial.println("");
...
void writeID( byte a[] ) {
  if ( !findID( a ) ) {     // Before we write to the EEPROM, check to see if we have seen this card before!
    ...
    successWrite();
    Serial.println(F("Ahora la tarjeta tiene acceso."));
  }
  else {
    failedWrite();
    Serial.println(F("Lo sentimos, ha ocurrido un error inesperado."));
  }
}
...
void deleteID( byte a[] ) {
  if ( !findID( a ) ) {     // Before we delete from the EEPROM, check to see if we have this card!
    ...
    successDelete();
    Serial.println(F("Se removio el acceso de la tarjeta."));
  }
}
